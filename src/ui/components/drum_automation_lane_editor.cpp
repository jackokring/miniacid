#include "drum_automation_lane_editor.h"
...
DrumAutomationLaneEditor::DrumAutomationLaneEditor(MiniAcid& mini_acid,
                                                   AudioGuard& audio_guard)
  : mini_acid_(mini_acid),
    audio_guard_(audio_guard),
    param_id_(DrumAutomationParamId::DrumEngine),
    cursor_x_(0),
    cursor_y_(0) {}

void DrumAutomationLaneEditor::setParamId(DrumAutomationParamId id) {
  param_id_ = id;
}

void DrumAutomationLaneEditor::withAudioGuard(const std::function<void()>& fn) {
  if (audio_guard_) {
    audio_guard_(fn);
    return;
  }
...
void DrumAutomationLaneEditor::clampCursor() {
  cursor_x_ = clampIndex(cursor_x_, kXSteps - 1);
  cursor_y_ = clampIndex(cursor_y_, ySteps() - 1);
}

void DrumAutomationLaneEditor::setCursorFromPoint(int x, int y) {
...
uint8_t DrumAutomationLaneEditor::cursorValue() const {
  int steps_y = ySteps();
  if (steps_y <= 1) return 0;
  const Parameter& param = mini_acid_.drumParameter(param_id_);
...
int DrumAutomationLaneEditor::valueToYIndex(uint8_t value) const {
  int steps_y = ySteps();
  if (steps_y <= 1) return 0;
  const Parameter& param = mini_acid_.drumParameter(param_id_);
  const AutomationLane* lane = mini_acid_.automationLaneDrum(param_id_);
...
int DrumAutomationLaneEditor::ySteps() const {
  const Parameter& param = mini_acid_.drumParameter(param_id_);
  const AutomationLane* lane = mini_acid_.automationLaneDrum(param_id_);
...
int DrumAutomationLaneEditor::xToPixel(int x) const {
  Rect bounds = graphBounds();
  if (bounds.w <= 1) return bounds.x;
  return bounds.x + (x * (bounds.w - 1)) / (kXSteps - 1);
}

int DrumAutomationLaneEditor::yIndexToPixel(int yIndex) const {
  Rect bounds = graphBounds();
...
Rect DrumAutomationLaneEditor::graphBounds() const {
...
bool DrumAutomationLaneEditor::removeNodeAtCursor() {
  AutomationLane* lane = mini_acid_.editAutomationLaneDrum(param_id_);
...
bool DrumAutomationLaneEditor::addNodeAtCursor() {
  AutomationLane* lane = mini_acid_.editAutomationLaneDrum(param_id_);
  if (!lane) return false;
  const Parameter& param = mini_acid_.drumParameter(param_id_);
...
bool DrumAutomationLaneEditor::toggleNodeAtCursor() {
  bool removed = false;
...
bool DrumAutomationLaneEditor::handleEvent(UIEvent& ui_event) {
...
void DrumAutomationLaneEditor::draw(IGfx& gfx) {
  ...
  for (int x = 0; x < kXSteps; ++x) {
    int px = xToPixel(x);
    drawLineColored(gfx, px, graph_bounds.y, px, graph_bounds.y + graph_bounds.h - 1,
                    COLOR_GRAY_DARKER);
  }
  ...
  if (mini_acid_.isPlaying() && current_step >= 0) {
    ...
    int play_x = bounds.x;
    if (graph_bounds.w > 1) {
      play_x = graph_bounds.x +
               static_cast<int>(play_pos * (graph_bounds.w - 1) / max_pos + 0.5f);
    }
  ...
  if (option_lane) {
    ...
    if (option_count > 0) {
      ...
      for (int i = 0; i < option_count; ++i) {
        const char* label = lane && lane->hasOptions() ? lane->optionLabelAt(i)
                                                       : param.optionLabelAt(i);
